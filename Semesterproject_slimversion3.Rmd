...
Caroline's moving pattern shows her staying in one place during the days (mostly between 6-9 am to around 6pm). That's most likely when she's sleeping. Movements, or better to say: changing of distances, are happening only at night, which was to be expected of a nocturnal animal. *(Lia)*
...
ggplot() +
  geom_rect(data = Relative_breaks,
            aes(xmin = start,
                ...
                fill = Phases),
            alpha = 0.2) +
  geom_line(data=relChange_all_3boars_day, aes(day,dist_perc, color=TierName)) + labs(y="Change in distance [%]", title="Daily relative change in distance from WSS-2015-01") + theme_classic() 
...
ggplot() +
  geom_rect(data = Relative_breaks,
            aes(xmin = start,
                ...
                fill = Phases),
            alpha = 0.2) +
  geom_line(data=relChange_all_3boars_week, aes(Week,dist_perc, color=TierName)) + labs(y="Change in distance [%]", title="Weekly relative change in distance from WSS-2015-01") + theme_classic() 
  #geom_vline(xintercept=20, linetype="dashed"),
