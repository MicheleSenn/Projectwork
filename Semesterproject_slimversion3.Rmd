...
Can a return pattern to the location of the Schreck be modelled?

## Data
Data about the "Schrecks", about the boars movement tracking and field data about the placement of the Schreck was used in this project.
...
The following graphs show Caroline's location around the bean field during active WSS-2015-01, first during standard mode and then during aggressive mode. These graphs seem to have an issue with scaling which makes it difficult to compare the spread of locations.
...
To conceptualize the active and inactive time of the Schreck, a background fill was used. *(Lia)*
...
*(Lia)* As the results show, all boars moved in a 0 to 1500 m radius around the Schreck before the sound started, moved around it in a 0 to 1000 m radius during standard mode and increased radius when switching to aggressive and then quiet mode (= after sounding). Caroline's and Olga's movements could be compared, whereas Sabine moved in a different way.
Distance was chosen to conceptualize movement, when plotting against the time on the x axis. The question is, if it can be considered **useful** in order to conceptualize an actual pattern. Additionally, validity of the relationships might not be general enough. With such a small analysis (3 boars with one Schreck), it could easily capture only a peculiarity of the current data. (2)

*(Lia)* The above explained topic was assessed by using another way of conceptualizing the distance: looking at the relative change of distance in relation to the last day or week (Idea from (4)). It could be considered as distinguishing between first order and second order effects (3).
Looking at the resulting graphs, it can't be concluded that this measure provides a more useful pattern. At least, the findings from the distance graphs can be reproduced this way (especially the similarity between Olga and Caroline) which could be considered as a validation step.

# 3.2 In Field vs. Not In Field
*(Lia)* The original concept to annotate the times when the boars were inside of the bean field resulted in a creation of point at "in Field" moments. The binary display of In vs. Not In didn't provide any useful pattern so it was omitted from the project. In the results, red points can be seen and followed, and interestingly, the boars were visiting the field even during Schreck time. Not even the aggressive mode during activity stopped them from returing to the field.
This concept could start the analysis of finding returing patterns, though validity

## 3.3 Smoothing methods
*(Lia)* Counting as an interesting finding was the fact that the boars seemed to reduce and increase their distance on a daily basis, which could relate to their behaviors (sleeping outside of the field and moving back during their active days). This semantic annotation is purely speculative, as it was not further analysed (5).
...
Subsetting and reducing sample size to every 150th observation was used for the result, mainly due to the highest degree of readability.

## 3.4 Validation
Validation was carried out derived from options in the Rykiel paper (1). It consisted mainly of replotting data either with a subset of the data or in comparison with other parameters (e.g. IDs oder boars).

## 3.5 Problems and solutions
Before actual conceptualization could take place, a lot of time was invested to find the affected boars, which was necessary to continue the work on the research question.

# Outlook / further steps
- Context of data (weather, maps, topological...) (3)
- Segmentation of trajectories so behavior patterns can be classified. Ruling out inactive times might show more distinct patterns or at least serve as validation method. (6)
