...
The distance of three boars named Caroline, Olga and Sabine to a Schreck in the region Fanel (WSS-2015-01) was visualized. The model depicts also the Schreck activity, which changed from standard to aggressive mode, and the moments when the boars spent time inside of the field where the Schreck was located.
Additionally, the relative change in distance was shown as a second order effect.  
Though the behavior could be modeled by distance in a simple way, it is still lacking validation and the amount of data might not have been enough to draw conclusions.
*(both)*

# Introduction
Due to an exceptionally high rate of reproduction and an abundant food supply, the wild boar population in Switzerland has been growing for centuries. As the population grows, so does the damage. According to Stefan Suter, the challenges of wild boar management cannot be met by hunting measures alone. For a successful wild boar management, a holistic approach is important. Prevention methods and limited access to energy-rich food will limit the population growth in wild boars. The wild boar deterrent developed at the ZHAW uses alarm and warning calls from wild boars to keep them out of the fields. Previous methods like electric fences and hunting are often not effective, time-consuming and expensive.[7]
*(Michele)*

To test the effectiveness of the developed deterrents around 150 wild boars were caught and marked in the test areas Fanel - a nature reserve at Lake Neuchâtel, Klingnau reservoir and upper Fricktal. Of these, 36 wild boars were fitted with GPS-GSM transmitter collars to document their spatial behaviour. In addition, the damage on the fields was determined with the help of drone aerial photographs. The knowledge gained in this way serves as the basis for a sustainable boar management.[7]
*(Michele)*

This project aims to develop a method for modelling the behaviour of wild boars who have been in contact with scare-off measures. Research questions are formulated, and it is described how those were investigated and implemented. The results will be presented and discussed in the light of the research questions. Potential further steps for future research will also be addressed. Problems and limitations encountered along the way and the solutions to overcome these, be it limitations with respect to the data sources, the tools or any other source of limitation will also be discussed.
*(Michele)*
...
The hypothesis is that their behavior can be modelled by their distance to the corresponding Schreck, as well as comparing the average time they spent in a certain field before the scare-off measure called "Schreck" was put into action, during and after. 
*(Michele)*
...
## 2.9 In the field vs. out of the field before, during and after Schreck action

In this approach the time of a boar inside a certain area is compared to the time spent outside of this area. The idea behind this approach was to gather information out of a rather large dataset that is relevant for a farmer and for the researcher, who but the scare-off measure called "Schreck" into place, to know if it has the effect of keeping the boars out of a certain area. Furthermore, this approach should provide some relative numbers with which a conclusion might be reached as to whether or not the scare-off measures have an effect on the boars. *(Michele)*
...
A binary approach in form of a inside vs. outside the field was used. The defined area is a certain field. This will be done for the period before the Schreck was put into action, during and after. The percentage of time spent inside the field vs. outside the field can then be compared between the timespan of before the Schreck was put into action, during and after.
*(Michele)*

As a first step the Schreck and boars for this approach had to be selected. It had to be Schrecks and boars were several animals were in the vicinity of a Schreck during its running period were there's also enough tracking data of the boars movement from the period before the Schreck was put into action and from the period after the Schreck was active. This was done as described above. Schreck WSS_2015_01 was selected and the boars Caroline, Sabine and Olga, also as described in more detail above.
*(Michele)*

Next the area were the boars move around was mapped on a background map to get an idea of the terrain where the boars moved around.
*(Michèle)*

```{r map boars movement areas on a background map}
library(terra)
...
tm_shape(pk100_BE) + 
  tm_rgb() +
  tm_shape(wildschwein_convex_hull) +
...
## In the field vs. out of the field before, during and after Schreck action

Schreck WSS-2015-01 is in a bean field (as described above). Caroline seems to be spending 0.03 % of her time in that field before the Schreck was put there, 2 % of her time during the Schrecks active period regardless if it's in aggressive mode or not and 0.1 % of her time after the Schrecks active period.

Sabine seems to be spending 0 % of her time in that field before the Schreck was put there, 0.9 % of her time during the Schrecks active period while it was in normal mode, 7 % of her time while the Schreck is in aggressive mode and 0.2 % of her time after the Schrecks active period.

Olga seems to be spending 0.4 % of her time in that field before the Schreck was put there, 2 % of her time during the Schrecks active period, 6 % of her time during the Schrecks active period in aggressive mode and 0.06 % of her time after the Schrecks active period.
*(Michèle)*

```{r Caroline in the field vs not in the field during SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Caroline in the field vs not in the field during SChreck time in aggressive mode, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
st_intersects(schreck5, forest)

fieldforschreck5 <- forest %>%
  filter(FieldID==195)

Caroline_Schrecktimeag <- wildschwein_sf %>%
  filter(TierName == "Caroline", DatetimeUTC > "2015-06-12", DatetimeUTC < "2015-07-01")

Caroline_field <- st_disjoint(fieldforschreck5,Caroline_Schrecktime)


Caroline_field_during <- Caroline_Schrecktimeag[Caroline_field[[1]],]

CarolineIntersect <- st_intersects(fieldforschreck5,Caroline_Schrecktimeag)

intersect <- Caroline_Schrecktimeag[CarolineIntersect[[1]],]
...
#append 
Caroline_field_inout_duringaggressiv <- bind_rows(Caroline_field_during,intersect)

  


#113 from 1822 Data points are in the field
#print(113/1822*100)
# during Schreck time she spends 2 % of her time in the field.

```

```{r Caroline in the field vs not in the field before SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Caroline in the field vs not in the field after SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

Caroline_Schrecktime_after <- wildschwein_sf %>%
  filter(TierName == "Caroline", DatetimeUTC > "2015-07-01")
...
#Data points are in the field
#print(43/37207*100)
# during Schreck time she spends 0.1 % of her time in the field.

```

```{r Sabine in the field vs not in the field during SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Sabine in the field vs not in the field during SChreck time in aggressive mode, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

Sabine_Schrecktimeag <- wildschwein_sf %>%
  filter(TierName == "Sabine", DatetimeUTC > "2015-06-12", DatetimeUTC < "2015-07-01")

Sabine_field <- st_disjoint(fieldforschreck5,Sabine_Schrecktimeag)


Sabine_field_during <- Sabine_Schrecktimeag[Sabine_field[[1]],]

SabineIntersect <- st_intersects(fieldforschreck5,Sabine_Schrecktimeag)

intersect <- Sabine_Schrecktimeag[SabineIntersect[[1]],]
...
#append 
Sabine_field_inout_duringaggressiv <- bind_rows(Sabine_field_during,intersect)
...
# Data points are in the field
#print(121/1824*100)
# during Schreck time she spends 7 % of her time in the field.

```

```{r Sabine in the field vs not in the field before SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Sabine in the field vs not in the field after SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

Sabine_Schrecktime_after <- wildschwein_sf %>%
  filter(TierName == "Sabine", DatetimeUTC > "2015-07-01")
...
#Data points are in the field
#print(5/2548*100)
# during Schreck time she spends 0.2 % of her time in the field.

```

```{r Olga in the field vs not in the field during SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Olga in the field vs not in the field during SChreck time in aggressive mode, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

Olga_Schrecktimeag <- wildschwein_sf %>%
  filter(TierName == "Olga", DatetimeUTC > "2015-06-12", DatetimeUTC < "2015-07-01")

Olga_field <- st_disjoint(fieldforschreck5,Olga_Schrecktimeag)


Olga_field_during <- Olga_Schrecktimeag[Olga_field[[1]],]

OlgaIntersect <- st_intersects(fieldforschreck5,Olga_Schrecktimeag)

intersect <- Olga_Schrecktimeag[OlgaIntersect[[1]],]
...
#append 
Olga_field_inout_duringaggressiv <- bind_rows(Olga_field_during,intersect)
...
#Data points are in the field
#print(113/1822*100)
# during Schreck time she spends 6 % of her time in the field.

```

```{r Olga in the field vs not in the field before SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Olga in the field vs not in the field after SChreck time, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

Olga_Schrecktime_after <- wildschwein_sf %>%
  filter(TierName == "Olga", DatetimeUTC > "2015-07-01")
...
#Data points are in the field
#print(11/18004*100)
# during Schreck time she spends 0.06 % of her time in the field.

```

```{r plot beforeduringafter in field percentage}

beforeduringafter <- data.frame(before = c ("Caroline"= 0.03, "Sabine"= 0, "Olga"= 0.4), during = c(2, 0.9, 2), aggressivemode = c(2, 7, 6), after = c(0.1, 0.2, 0.06))

dd <- cbind(beforeduringafter$before,beforeduringafter$during, beforeduringafter$aggressivemode, beforeduringafter$after)

barplot(dd, beside=T,  names.arg=c('before','during','aggressivemode','after'), legend = c("Caroline", "Sabine", "Olga"),xlim = c(0, 21), ylab = "time spent in field (%)")

```

To see if the boars presence or absence in the field was influenced by the Schreck or by the crop type, the percentage of the boar spent in a certain crop was plotted.
It doesn't look like the boars favor the bean field, they seem to prefer oats.
*(Michèle)*

```{r preference of the boars in crop type}

selectedboars <- wildschwein_sf %>%
  filter(TierName == c("Caroline", "Sabine", "Olga"))

boarsforest <-  st_join(selectedboars, forest)

library(lubridate)
library(forcats)

wildschwein_smry <- boarsforest %>%
...
ggplot(wildschwein_smry, aes(hour,perc, fill = Frucht)) +
  ...
  facet_wrap(~TierName ) +
  theme_light() +
  labs(title = "Percentages of samples in a given crop per hour")

```

In order to validate this their whereabouts were plotted before, during and after the Schrecks active period. To visualize whether or not they were inside the field in which the Schreck was placed, the area of the field is marked in the plot.
*(Michèle)*
...
ggarrange(
  ...
  ggplot(fieldforschreck5)+geom_sf(aes(fill=Frucht)) +
        geom_sf(data=Caroline_Schrecktimeag, size = 0.2) + labs(title="Caroline during aggressive mode") + theme_classic(), 
  ggplot(fieldforschreck5)+geom_sf(aes(fill=Frucht)) +
...
ggarrange(
  ...
  ggplot(fieldforschreck5)+geom_sf(aes(fill=Frucht)) +
        geom_sf(data=Sabine_Schrecktimeag, size = 0.2) + labs(title="Sabine during aggressive mode") + theme_classic(), 
  ggplot(fieldforschreck5)+geom_sf(aes(fill=Frucht)) +
...
ggarrange(
  ...
  ggplot(fieldforschreck5)+geom_sf(aes(fill=Frucht)) +
        geom_sf(data=Olga_Schrecktimeag, size = 0.2) + labs(title="Olga during aggressive mode") + theme_classic(), 
  ggplot(fieldforschreck5)+geom_sf(aes(fill=Frucht)) +
...
# 3. Discussion
This project aimed to develop a method for modelling the behaviour of wild boars who have been in contact with scare-off measures. 
Once a scare-off measure has been set into action, it needs to be examined weather or not it has an effect. Therefore, the following research questions were formulated:  
**How can the behavior of the boars who had been in contact with a Schreck be modelled?**  


**How can a return pattern to the location of the Schreck be modelled?**

The hypothesis was that their behavior can be modelled by their distance to the corresponding Schreck, as well as comparing the average time they spent in a certain field before the scare-off measure called "Schreck" was put into action, during and after. 
*(Michele)*
...
The original concept to annotate the times when the boars were inside of the bean field resulted in a creation of point at "in Field" moments. The binary display of In vs. Not In didn't provide any useful pattern. In the results, red points can be seen and followed, and interestingly, the boars were visiting the field even during Schreck time. Not even the aggressive mode during activity stopped them from returning to the field.  
This concept could start the analysis of finding returning patterns, though more validation would need to be executed. *(Lia)* 

In this approach the time of a boar inside a certain area was compared to the time spent outside of this area, which should produce a conclusion out of a large dataset which is relevant and understandable for a farmer as well as for the researcher, who put the scare-off measures into place. It should provide a conclusive answer on whether or not the Schreck has the effect of keeping the boars out of a certain area. *(Michele)* 

The question which this approach aimed to answer is:
How much time does a boar spend in the field before the Schreck (in %), during and after?

The In Field vs. Not In Field approach did reach the goal of answering this question. 
All three boars seem to spend more time in the bean field when the Schreck was on, compared to before the Schreck was put there and even more once the Schreck was put into aggressive mode. The time spent in the field remained higher also after the Schreck was removed, compared to before the Schreck was even put into place. Not only does the Schreck seem to fail to reduce the percentage of the boars spent in the field, it seems to attract them. They spend more time in the field once the Schreck is on and even more when it goes into aggressive mode and keep coming back even after the Schreck was removed. *(Michele)*

This method delivers relative numbers with which a conclusion can be reached. However for those percentages to be valid enough to reach a statistical conclusion and to decide if an observed effect is significant or not, more data would be needed. Several animals would need to be tracked during the same time period in the same area, so that the data could be compared and have some statistical weight and that finally an effect could be called significant. This poses a challenge however, since several animals would have to be caught and have a tracker attached to them at the same time, which might be unrealistic. Furthermore, those animals would have to move around in the same area, which is something the researchers have no control over. Therefore the data gained by tracking wild boars is difficult to interpret and should be weighted carefully. There might be a great number of reasons for an observed effect, such as the personality of the boar itself, the geographic region, the season, etc. 
This approach can only deliver meaningful and reliable results, if a statistically significant number of individuals is tracked during the same time period and the same area. 
If that is not possible, drone data on the caused damage might the more reliable indicator on the effectiveness of the Schrecks.
*(Michele)*
...
## 3.4 Validation
Validation was carried out derived from options in the Rykiel paper (1). It consisted mainly of replotting data either with a subset of the data or in comparison with other parameters (e.g. IDs or boars). *(Lia)* 

## 3.5 Problems and possible solutions
- Before actual conceptualization could take place, a lot of time was invested to find the affected boars and which Schrecks they were affected by, a necessary task to continue the work on the research question. Analyzing data from another researcher without having accompanied the process means that more pre-processing and exploratory data analysis needs to be done. On the other hand, it might lead to finding unexpected patterns, because no expectations were present.  
...
- One main issue was the difficulty to estimate whether the animals would hear the Schrecks or not - does it depend on distance or is it more related to topological or environmental factors (e.g. wind, density of forest / field)? 
*(Lia)*
- Another big issue was that not enough individuals were tracked over the same time period, in the same area. It is therefore impossible to say if an observed effect is due to a coincidence, like a change in movement pattern of one individual boar, or if it is an effect of the Schreck.
*(Michele)*
...
- Could the displacement as an additional alternative to distance and change in distance provide alternative and more meaningful patterns? *(Lia)*
...
- Extend the analysis with the existing data set to other boars and Schrecks (if at all possible). *(Lia)*
...
(7)Sutter, S. (2019).Wildschweinschäden mit akustischer Methode verhindern: https://www.zhaw.ch/storage/hochschule/medien/bildmaterial/Medienmitteilung_Wildschweinschreck.pdf
