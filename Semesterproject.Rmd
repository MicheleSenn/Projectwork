...
#die Monate dazwischen setze ich nun manuell dazu...
fanel_months_all <- fanel_months %>%
  add_row(id="WSS_2014_04", name="month_2", month_year = "6 - 2014") %>%
  add_row(id="WSS_2014_04", name="month_3", month_year = "7 - 2014") %>%
  add_row(id="WSS_2014_04", name="month_4", month_year = "8 - 2014") %>%
  add_row(id="WSS_2014_04", name="month_5", month_year = "9 - 2014") %>%
  add_row(id="WSS_2014_05", name="month_2", month_year = "6 - 2014") %>%
  add_row(id="WSS_2014_05", name="month_3", month_year = "7 - 2014") %>%
  add_row(id="WSS_2014_05", name="month_4", month_year = "8 - 2014") %>%
  add_row(id="WSS_2014_05", name="month_5", month_year = "9 - 2014") %>%
  add_row(id="WSS_2014_06", name="month_2", month_year = "6 - 2014") %>%
  arrange(id, month_year)
  
agenda_fanel_months <- st_join(agenda_fanel,fanel_months_all,by="id") %>%
  select(-id.y,-name)
...
join_fanel_wildschwein_months <- inner_join(fanel_months_all,wildschwein_months_years,by = "month_year")  
knitr::kable(join_fanel_wildschwein_months %>%
               group_by(year,TierName) %>%
         summarize(n=n())) #diese Tiere kommen möglicherweise beim Schreck vorbei, während er ON ist (zeitlich)
```
...
#Nun muss die Distanz zum jeweiligen Schreck gerechnet und dargestellt werden.


#Ich schaue mir einmal Ueli ein wenig genauer an
Ueli <- wildschwein_sf %>%
  filter(TierName == "Ueli" & lubridate::year(DatetimeUTC) == 2014)
ggplot(Ueli,aes(DatetimeUTC,moonilumination)) + geom_point()

Ueli_Schreck <- join_fanel_wildschwein_months %>%
  filter(TierName =="Ueli" & year == 2014)

#alle Schrecks in Spalten mit der jeweiligen Geometrie. Damit können sie nachher mit den Wildschwein locations verbunden werden
schreck_locations_pivot <- schreck_locations_sf_fanel %>%
  select(id) %>%
  tidyr::pivot_wider(names_from="id", values_from="geometry")
#hier komme ich noch nicht weiter --> will die Spalten aus schreck_locations_pivot hier anfügen oder zumindest eine davon (am liebsten ohne join)
#Ueli_distance <- Ueli %>%
#  bind_cols(schreck_locations_pivot) %>%
#  mutate(distanceToSchreck = CalculateEuclideanDistance(WSS_2014_04_geometry,geometry))
```
<!-- the following is just a placeholder text, remove it!-->
