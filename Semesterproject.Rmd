...
ggplot(wildschwein_schreck1_close, aes(DatetimeUTC,distance, color=TierName)) + geom_line()
```
```{r Ueli only with Schreck1}
(Ueli_schreck1 <- wildschwein_schreck1_close %>%
...
(Ueli_schreck1 <- wildschwein_schreck1 %>%
  dplyr::filter(TierName=="Ueli") %>%
  ggplot(aes(DatetimeUTC,distance)) + geom_line())
# he came back after a while, even almost to 1000 m to the Schreck
```
