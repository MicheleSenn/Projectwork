...
# Step 3
In order to understand which animals were active during the period of "Schreck time", the boar data was filtered by the dates in which the first Schreck (WSS_2014_04) was active.
For the affected boars, a distance between their coordinates and the Schreck coordinate could be calculated using st_distance().
For a reduction in data points, all distances bigger than 2000m were removed. The remaining distances were plotted against the DatetimeUTC column.
5 animals were close to Schreck 1 during "Schreck time".
...
wildschwein_schreck1_long <- filter(wildschwein_sf,DatetimeUTC>schreck1$datum_on)
...
wildschwein_schreck1_long$distance <- as.numeric(st_distance(wildschwein_schreck1_long,schreck1)[,1])
...
#whole time
wildschwein_schreck1_close_long <-
  wildschwein_schreck1_long%>%
  filter(distance < 200)
...
SabineIsablle_distance <- wildschwein_schreck1_close_long %>%
