...
As can be seen, Schreck 5 is in a bean field. Caroline made some visits during Schreck time.
...
```{r in the field vs not in the field}
...
distance5_Caroline_longerThanSchrecktime <- wildschwein_sf %>%
  filter(TierName == "Caroline")

distance5_Caroline_longerThanSchrecktime$distance_Schreck5 <-
  as.numeric(st_distance(distance5_Caroline_longerThanSchrecktime$geometry,schreck5)[,1])

ggplot(forest_bohnen195)+geom_sf(aes(fill=Frucht)) + geom_sf(data=distance5_Caroline_longerThanSchrecktime)

intersection_195andSchreck5_long <- st_intersects(forest_bohnen195,distance5_Caroline_longerThanSchrecktime)
class(intersection_195andSchreck5)

inter_sel_5_long <- distance5_Caroline_longerThanSchrecktime[intersection_195andSchreck5_long[[1]],]

ggplot(forest_bohnen195) + geom_sf(aes(fill=Frucht))+ geom_sf(data=inter_sel_5_long) + geom_sf(data=schreck5, aes(size=5)) + labs(title="Caroline's positions in the bean field during the whole time")
  #during Schreck time, Carolina still visited the bean field

#ich will jetzt das Feld definieren. Im Feld drin = TRUE --> alle in inter_sel_5_long
#für die "nicht enthaltenen" kann ich einen anti-join nutzen oder das Subsetting mit disjoint

disjoint_195andSchreck5_long <- st_disjoint(forest_bohnen195,distance5_Caroline_longerThanSchrecktime)
class(intersection_195andSchreck5_long)

disjoint_caroline_5_long <- distance5_Caroline_longerThanSchrecktime[disjoint_195andSchreck5_long[[1]],]

#disjoint anschauen --> ok!
ggplot(forest_bohnen195) + geom_sf(aes(fill=Frucht))+ geom_sf(data=disjoint_caroline_5_long) + geom_sf(data=schreck5, aes(size=5)) + labs(title="Caroline's positions outside bean field during the whole time")

#nun die ausserhalb mit 0 und die innerhalb mit 1 ergänzen
disjoint_caroline_5_long$inField <- 0
inter_sel_5_long$inField <- 1
#append the two data sets again with their new columns
Caroline_Schreck5_withField_long <- bind_rows(disjoint_caroline_5_long,inter_sel_5_long)

#this is now the pattern during Schreck time --> red points don't make a lot of sense yet
ggplot() + geom_point(data=Caroline_Schreck5_withField_long,aes(DatetimeUTC,distance_Schreck5)) +
  geom_point(data=inter_sel_5_long,aes(DatetimeUTC,inField, colour="red")) +
  labs(title="Caroline's distance to Schreck 5 during whole tracking time. Red points mean: in the bean field")
```
...
distance6_Olga_Schrecktime <-wildschwein_sf %>%
  filter(TierName == "Olga", DatetimeUTC > "2015-05-22", DatetimeUTC < "2015-06-12")
...
```{r double checking concurrence}

#wildschweine_2017 <- wildschwein_sf %>%
#  filter(lubridate::year(DatetimeUTC)==2017) #nice, keine Wildschweine wurden während Schrecktime im Jahr 2017 getrackt!

#rm(wildschweine_2017)

wildschweine_2016 <- wildschwein_sf %>%
  filter(lubridate::year(DatetimeUTC)==2016) %>%
  group_by(TierName) %>%
  summarise()
#Miriam seems to be promising as she was tracked for a long time as well, and Amos
```

```{r Miriam or Amos Schreck 11-14}
schreck11 <- agenda_fanel[11,]
...
#next step
How much time does a boare spend in the field before the Schreck (in %), during and after? 
...
For Schreck 5 and the boars Olga, Sabine and Caroline will be further examined, because they were in the area as well as tracked before and after. 
